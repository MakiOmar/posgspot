...
class OpeningStockController extends Controller
{
    ...
    /**
     * Backup of previous save logic with try/catch kept for reference during debugging.
     *
     * @param  \Illuminate\Http\Request  $request
     * @return \Illuminate\Http\Response|array
     */
    protected function saveWithTryCatchBackup(Request $request)
    {
        if (! auth()->user()->can('product.opening_stock')) {
            abort(403, 'Unauthorized action.');
        }
    ...
    public function save(Request $request)
    {
        ...
        if (! empty($product) && $product->enable_stock == 1 && ! empty($opening_stocks)) {
            $tax_percent = ! empty($product->product_tax->amount) ? $product->product_tax->amount : 0;
            $tax_id = ! empty($product->product_tax->id) ? $product->product_tax->id : null;

            $financial_year_start = $request->session()->get('financial_year.start');
            $default_transaction_date = \Carbon::createFromFormat('Y-m-d', $financial_year_start)->toDateTimeString();

            DB::transaction(function () use ($opening_stocks, $locations, $product, $tax_percent, $tax_id, $default_transaction_date, $business_id, $user_id) {
                foreach ($opening_stocks as $location_id => $value) {
                    if (! array_key_exists($location_id, $locations)) {
                        continue;
                    }
                    ...
                    foreach ($value as $vid => $purchase_lines_data) {
                        foreach ($purchase_lines_data as $k => $pl) {
                            ...
                            $lot_number = ! empty($pl['lot_number']) ? $pl['lot_number'] : null;
                            $purchase_line_note = ! empty($pl['purchase_line_note']) ? $pl['purchase_line_note'] : null;
                            $resolved_transaction_date = ! empty($pl['transaction_date']) ? $this->productUtil->uf_date($pl['transaction_date'], true) : $default_transaction_date;
