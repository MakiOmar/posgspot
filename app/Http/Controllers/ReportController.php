...
class ReportController extends Controller
{
    ...
    public function getStockReport(Request $request)
    {
        ...
        $units = Unit::where('business_id', $business_id)
                            ->pluck('short_name', 'id');
        $business_locations = BusinessLocation::forDropdown($business_id, true);
        $stock_location_choices = BusinessLocation::forDropdown($business_id, false);
        $default_stock_location = collect($stock_location_choices)->keys()->first();
        if (! is_null($default_stock_location)) {
            $default_stock_location = (string) $default_stock_location;
        }
    ...
    public function getproductSellReport(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            // TEMP DEBUG: add &debug_psr=1 to the report ajax URL to log where rows drop.
            // Logs staged counts per join so we can see which join/filter empties results.
            if ($request->boolean('debug_psr')) {
                $base = \DB::table('transactions as t')
                    ->where('t.business_id', $business_id)
                    ->where('t.type', 'sell')
                    ->where('t.status', 'final');
                if (! empty($start_date) && ! empty($end_date)) {
                    $base->where('t.transaction_date', '>=', $start_date)
                        ->where('t.transaction_date', '<=', $end_date);
                }
                if ($permitted_locations != 'all') {
                ...
                $stage = fn ($joins) => (clone $base)->when($joins, $joins)->count();

                \Log::info('PSR_DEBUG product-sell-report', [
                    ...
                    'stage_counts' => [
                        '1_transactions' => $stage(null),
                        '2_with_sell_lines' => $stage(fn ($q) => $q
                            ->join('transaction_sell_lines as tsl', 'tsl.transaction_id', '=', 't.id')),
                        '3_with_variations' => $stage(fn ($q) => $q
                            ->join('transaction_sell_lines as tsl', 'tsl.transaction_id', '=', 't.id')
                            ->join('variations as v', 'tsl.variation_id', '=', 'v.id')),
                        '4_with_product_variations' => $stage(fn ($q) => $q
                            ->join('transaction_sell_lines as tsl', 'tsl.transaction_id', '=', 't.id')
                            ->join('variations as v', 'tsl.variation_id', '=', 'v.id')
                            ->join('product_variations as pv', 'v.product_variation_id', '=', 'pv.id')),
                        '5_with_contacts_INNER' => $stage(fn ($q) => $q
                            ->join('transaction_sell_lines as tsl', 'tsl.transaction_id', '=', 't.id')
                            ->join('variations as v', 'tsl.variation_id', '=', 'v.id')
                            ->join('product_variations as pv', 'v.product_variation_id', '=', 'pv.id')
                            ->join('contacts as c', 't.contact_id', '=', 'c.id')),
                        '6_with_products' => $stage(fn ($q) => $q
                            ->join('transaction_sell_lines as tsl', 'tsl.transaction_id', '=', 't.id')
                            ->join('variations as v', 'tsl.variation_id', '=', 'v.id')
                            ->join('product_variations as pv', 'v.product_variation_id', '=', 'pv.id')
                            ->join('contacts as c', 't.contact_id', '=', 'c.id')
                    ...
                    'transactions_with_null_contact_id' => (clone $base)->whereNull('t.contact_id')->count(),
                    'final_query_count' => (clone $query)->count(),
    ...
    public function itemsReport()
    {
        ...
        $max_range_months = max((int) config('constants.items_report_max_months', 1), 1);
        $now = Carbon::now();
        $default_range_end = $now->copy()->endOfDay();
        $default_range_start = $now->copy()->subDays($default_range_days - 1)->startOfDay();
        $max_default_end = $default_range_start->copy()->addMonths($max_range_months)->endOfDay();
        if ($default_range_end->greaterThan($max_default_end)) {
            $default_range_end = $max_default_end;
        }
        ...
        if (request()->ajax()) {
            ...
            $clampRange = function (?string $start, ?string $end) use ($max_range_months) {
                ...
                try {
                    ...
                    $endCarbon = $startCarbon->copy()->endOfDay();
                }
                ...
                $maxEnd = $startCarbon->copy()->addMonths($max_range_months)->endOfDay();
                if ($endCarbon->greaterThan($maxEnd)) {
                    $endCarbon = $maxEnd;
                }
            ...
            [$purchaseStartCarbon, $purchaseEndCarbon] = $clampRange($purchase_start, $purchase_end);
            [$saleStartCarbon, $saleEndCarbon] = $clampRange($sale_start, $sale_end);
            ...
            \Log::debug('Items report pagination debug.', [
                ...
                'business_id' => $business_id,
                'purchase_start' => optional($purchaseStartCarbon)->toDateTimeString(),
                'purchase_end' => optional($purchaseEndCarbon)->toDateTimeString(),
                'sale_start' => optional($saleStartCarbon)->toDateTimeString(),
                'sale_end' => optional($saleEndCarbon)->toDateTimeString(),
            ...
            if ($purchaseStartCarbon && $purchaseEndCarbon) {
                $query->whereBetween('purchase.transaction_date', [$purchaseStartCarbon, $purchaseEndCarbon]);
            }
            if ($saleStartCarbon && $saleEndCarbon) {
                $query->where(function ($q) use ($saleStartCarbon, $saleEndCarbon) {
                    $q->whereBetween('sale.transaction_date', [$saleStartCarbon, $saleEndCarbon])
                      ->orWhereBetween('stock_adjustment.transaction_date', [$saleStartCarbon, $saleEndCarbon]);
                });
            }
