...
class StorefrontSettingController extends Controller
{
    ...
    public function update(Request $request)
    {
        ...
        $validated = $request->validate([
            ...
            'geidea_mode' => 'nullable|in:test,live',
            'geidea_region' => 'nullable|in:EGY-PROD,KSA-PROD,UAE-PROD',
            'geidea_currency' => 'nullable|string|max:8',
            'geidea_language' => 'nullable|in:en,ar',
            'geidea_ui_mode' => 'nullable|in:modal,dropin',
            'geidea_hpp_profile' => 'nullable|in:simple,compressed',
            'geidea_logo_url' => 'nullable|string|max:500',
            'geidea_header_color' => ['nullable', 'string', 'max:16'],
            'geidea_hide_geidea_logo' => 'nullable|boolean',
            'geidea_show_email' => 'nullable|boolean',
            'geidea_show_phone' => 'nullable|boolean',
            'geidea_show_address' => 'nullable|boolean',
            'geidea_receipt_page' => 'nullable|boolean',
            'geidea_test_public_key' => 'nullable|string|max:191',
            'geidea_test_api_password' => 'nullable|string|max:500',
            'geidea_live_public_key' => 'nullable|string|max:191',
            'geidea_live_api_password' => 'nullable|string|max:500',
        ...
        $payload = [
            ...
            'gateway' => [
                ...
                'geidea' => [
                    'mode' => ($validated['geidea_mode'] ?? 'test') === 'live' ? 'live' : 'test',
                    'region' => $validated['geidea_region'] ?? 'EGY-PROD',
                    'currency' => strtoupper((string) ($validated['geidea_currency'] ?? 'EGP')) ?: 'EGP',
                    'language' => ($validated['geidea_language'] ?? 'en') === 'ar' ? 'ar' : 'en',
                    'ui_mode' => ($validated['geidea_ui_mode'] ?? 'modal') === 'dropin' ? 'dropin' : 'modal',
                    'hpp_profile' => ($validated['geidea_hpp_profile'] ?? 'simple') === 'compressed' ? 'compressed' : 'simple',
                    'logo_url' => $validated['geidea_logo_url'] ?? '',
                    'header_color' => $validated['geidea_header_color'] ?? '',
                    'hide_geidea_logo' => $request->boolean('geidea_hide_geidea_logo'),
                    'show_email' => $request->boolean('geidea_show_email'),
                    'show_phone' => $request->boolean('geidea_show_phone'),
                    'show_address' => $request->boolean('geidea_show_address'),
                    'receipt_page' => $request->boolean('geidea_receipt_page'),
                    'test_public_key' => $validated['geidea_test_public_key'] ?? '',
                    'test_api_password' => $validated['geidea_test_api_password'] ?? null,
                    'live_public_key' => $validated['geidea_live_public_key'] ?? '',
                    'live_api_password' => $validated['geidea_live_api_password'] ?? null,
                ],
            ],
            'social' => [
