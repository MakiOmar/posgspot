...
| GET | `/locations` | **All active** POS `business_locations` (not only selling locations). `address` uses **Storefront display address** when set, else landmark/city/state/country/zip. Includes `is_selling_location` (in Settings → selling locations), `enable_pickup`, coords, `maps_url`, and `email_encoded` (base64, not raw). Powers footer, contact, store locator; checkout pickup should filter to `is_selling_location && enable_pickup`. |
...
- **Footer menus** (`footer`) — contact column title + up to 3 link columns (EN/AR labels, path or absolute URL); public API returns locale-resolved titles/labels. Qwik footer column 1 lists all active locations from `GET /locations` (display address + Visit us / Call us) and social/WhatsApp; columns 2–4 render `footer.columns`.
...
- Public `GET /locations` returns all active POS locations (plus `is_selling_location`); uses `email_encoded` per location (no raw `email` field)
