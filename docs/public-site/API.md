...
| GET | `/digital/games/{id}` | Single game detail + POS `skus`. Platform stock fields (`ps4_*` / `ps5_*`) are integers; missing stock is `0` (no cross-platform fallback). Optional `description` is Accounts HTML sanitized via `StorefrontHtmlSanitizer` (null when empty). |
| GET | `/digital/card-categories` | Gift card categories + POS `skus`. |
| POST | `/digital/check-stock` | Accounts game stock check — body `{ game_id, type: primary\|secondary, platform: 4\|5 }`. 200 when Accounts `is_available` is true (quantity may be omitted). 422 `{ message: "This offer is out of stock." }` when unavailable. |
