...
| `promo_banner` | `image_url`, `title`, `link` — one banner per section; insert multiple to stack |
...
- **Homepage sections** (`homepage_sections`) — ordered section builder on Storefront Settings → Homepage tab; public `GET /homepage` returns enabled sections only (`hero_slider`, `promo_tiles`, `video`, `promo_banner`, `promo_banners` legacy, `featured_products`, `top_categories`, `category_shelves` legacy, `category_shelf`, `brand_slider`, `bestsellers`, `recently_viewed`). Prefer `promo_banner` (fields in builder) over legacy `promo_banners`, and `category_shelf` over legacy `category_shelves`. Hero/promo/video/banner media live in section settings; featured still uses product flags
