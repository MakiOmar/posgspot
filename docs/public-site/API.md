...
| GET | `/digital/games/featured?count=&product_type=` | Featured games for **both** platforms (Accounts `GET /api/games/featured`). `count` (default 10, max 50) is passed to Accounts as candidates per platform; response `tiles[]` is **unique by game id**, PS5 preferred, hard-capped to `count` total. Returns `{ count, product_type, games: { "4": [], "5": [] }, tiles: [{ id, image_url, href, label, game_id, platform }], skus }`. Homepage `promo_tiles` uses this internally. 503 when digital off. |
