...
| `trust_badges` | `items[]` → `{ id, icon_kind (image\|svg), icon_url, icon_color, title, description }` (icons are media URLs only; no inline `svg_markup`) |
