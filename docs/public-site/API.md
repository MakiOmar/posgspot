...
- **Homepage sections** (`homepage_sections`) — ordered section builder on Storefront Settings → Homepage tab; public `GET /homepage` returns enabled sections only (`hero_slider`, `promo_tiles`, `video`, `promo_banners`, `featured_products`, `top_categories`, `category_shelves`, `category_shelf`, `brand_slider`, `bestsellers`, `recently_viewed`). Prefer `category_shelf` (pick a category) over legacy `category_shelves`. Hero/promo/video media live in section settings; featured still uses product flags
