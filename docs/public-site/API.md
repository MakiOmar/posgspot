...
| GET | `/settings` | Business + storefront public settings (includes `sale_badge`, `catalog.show_availability_on_cards`, `payment_icons`, `about.team`, `favicon_url`, `logo_url`, `shop_menu.physical`, `footer`, `banners`, `couriers.bosta.enabled`, `digital.enabled`). Optional `?shell=1` omits `about.team` (empty array) for layout shell payloads. Responses send `Cache-Control: public, max-age=60` + `ETag`. |
...
| GET | `/digital/games/{id}` | Single game detail + POS `skus`. Platform stock fields (`ps4_*` / `ps5_*`) are integers; missing stock is `0` (no cross-platform fallback). |
