...
| GET | `/digital/games?platform=&page=&q=&product_type=` | Digital games catalog (Accounts proxy). `platform` = `4` (PS4) or `5` (PS5). Optional `product_type` = `game` (default) or `subscription` (PS Plus). Optional `q` filters by title/code. Returns normalized `games[]` (incl. `product_type`, `full_*` offer fields) + POS `skus` map. Offer statuses are true only when that platform offer is available **and** stock > 0. 503 when `digital.enabled` is off. |
| GET | `/digital/games/{id}` | Single game detail + POS `skus`. Platform stock fields (`ps4_*` / `ps5_*` for primary/secondary/full) are integers; missing stock is `0` (no cross-platform fallback). Optional `description` (sanitized HTML), `gallery[]` (`id`, absolute `url`, `sort_order`), `reviews` (`average`, `count`, `items[]` with stars/comment/reviewer_name), and `product_type`. |
