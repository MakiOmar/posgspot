$(document).ready(function() {
    ...
    register_report_table = $('#register_report_table').DataTable({
        ...
        columns: (function () {
            ...
            for (var i = 1; i <= 30; i++) {
                cols.push({ data: 'total_custom_pay_' + i, name: 'total_custom_pay_' + i, searchable: false });
            }
            cols.push(
        ...
        "footerCallback": function ( row, data, start, end, display ) {
            var total_card_payment = 0, total_cheque_payment = 0, total_cash_payment = 0;
            var total_bank_transfer_payment = 0, total_other_payment = 0, total_advance_payment = 0;
            var total_custom_pay = [];
            for (var i = 1; i <= 30; i++) { total_custom_pay[i] = 0; }
            var total = 0;
            for (var r in data) {
                ...
                for (var i = 1; i <= 30; i++) {
                    var col = data[r]['total_custom_pay_' + i];
                    total_custom_pay[i] += col && $(col).data('orig-value') ? parseFloat($(col).data('orig-value')) : 0;
                }
            ...
            for (var i = 1; i <= 30; i++) {
                $('.footer_total_custom_pay_' + i).html(__currency_trans_from_en(total_custom_pay[i]));
            }
