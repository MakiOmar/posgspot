...
export const CustomerQrCode = component$<CustomerQrCodeProps>(({ name, email, mobile }) => {
  ...
  return (
    <div class="account-qr">
      <h2>Your QR code</h2>
      <p class="footer-muted account-qr-hint">
        Scan to save your name, email and mobile as a contact.
      </p>
      <div class="account-qr-frame" aria-hidden={!dataUrl.value}>
        {dataUrl.value ? (
          ...
          <span class="account-qr-placeholder footer-muted">Generating…</span>
        )}
      </div>
    </div>
  );
